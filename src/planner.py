...
google_places_key = "AIzaSyB7AQIZKGZHgsk1mYoEOCqU280BIF7D42o"
google_client = googlemaps.Client(google_places_key)

# Construct the client object
client = foursquare.Foursquare(client_id='GCNY3EPNT4XPHLXKIHHYO5VZD1TC3D5QPMBKFWLMCWAWWPGP', client_secret='GFUH3CBHMOHV4HVPZOX5CTIDDNYOAF1J4FPUT50WTINDYTLK')
...
def build_text_response(city, route_sequence, matrix, places):
	...
	text_response += 'Your day starts at ' + places[0]["name"] + ", "
	text_response += 'and includes amazing places to visit such as '
	text_response += ", ".join(map(lambda x: x["name"], places[1:-1]))
	text_response += " and your trip finally comes to an end with you visiting " + places[-1]["name"] + "."
